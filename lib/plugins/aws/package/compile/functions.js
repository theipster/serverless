...
class AwsCompileFunctions {
    ...
    if (destinations) {
      ...
      if (destinations.onSuccess) {
        ...
        destinationConfig.OnSuccess = {
          Destination: this.isFunctionAddress(destinations.onSuccess)
      ...
      if (destinations.onFailure) {
        ...
        destinationConfig.OnFailure = {
          Destination: this.isFunctionAddress(destinations.onFailure)
  ...
  isFunctionAddress(target) {
    return typeof target === 'string' && !target.startsWith('arn:');
  }

  // Memoized in a construtor
  ensureTargetExecutionPermission(target) {
    ...
    const action = (() => {
      const resolveResourceType = logicalId => {
        const userResources = this.serverless.service.initialServerlessConfig.resources;
        if (userResources && 'Resources' in userResources && logicalId in userResources.Resources) {
          return userResources.Resources[logicalId].Type;
        }
        throw new this.serverless.classes.Error(`Unrecognized destination target ${logicalId}`);
      };

      /* eslint-disable default-case -- Unsupported targets should fall through */
      switch (typeof target) {
        case 'object':
          if ('Ref' in target) {
            switch (resolveResourceType(target.Ref)) {
              case 'AWS::SNS::Topic':
                return 'sns:Publish';
            }
          } else if ('Fn::GetAtt' in target && target['Fn::GetAtt'][1] === 'Arn') {
            switch (resolveResourceType(target['Fn::GetAtt'][0])) {
          ...
          // TODO: enable support for other intrinsic functions, e.g. Fn::Sub
          break;

        case 'string':
          if (this.isFunctionAddress(target)) return 'lambda:InvokeFunction';
          if (target.includes(':function:')) return 'lambda:InvokeFunction';
          if (target.includes(':sqs:')) return 'sqs:SendMessage';
          if (target.includes(':sns:')) return 'sns:Publish';
          if (target.includes(':event-bus/')) return 'events:PutEvents';
          break;
      }
      /* eslint-enable default-case */

      throw new this.serverless.classes.Error(`Unsupported destination target ${target}`);
    })();
    ...
    const resource = (() => {
      ...
      if (this.isFunctionAddress(target)) {
        return {
          'Fn::Sub': `arn:\${AWS::Partition}:lambda:\${AWS::Region}:\${AWS::AccountId}:function:${
            this.serverless.service.getFunction(target).name
          }`,
        };
      }
